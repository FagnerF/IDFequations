...
#' @export
ScriptIDFCLIMBra <-
  function(CLIMBra, Station, LatSR, LonSR, Directory) {
    tmp <- NULL
    ...
    if (CLIMBra == "ssp245") {
      cat("== Downloading the raster file ==", "\n")
      cat("Wait...", "\n")

      url <-
        "https://download.scidb.cn/download?fileId=63093950980594780dcfccb0&dataSetType=personal&fileName=EC-EARTH3-pr-ssp245.nc"

      download.file(url, paste0(Directory, "EC-EARTH3-pr-ssp245.nc"), method = "curl")

      cat("\014")

    } else if (CLIMBra == "ssp585") {
      cat("== Downloading the raster file ==", "\n")
      cat("Wait...", "\n")

      url <-
        "https://download.scidb.cn/download?fileId=63093950980594780dcfccb0&dataSetType=personal&fileName=EC-EARTH3-pr-ssp585.nc"


      download.file(url, paste0(Directory, "EC-EARTH3-pr-ssp585.nc"), method = "curl")
    ...
    nc <-
      terra::rast(paste0(Directory, "EC-EARTH3-pr-", CLIMBra, ".nc"))

    NamesDatanc <-
      raster::stack(paste0(Directory, "EC-EARTH3-pr-", CLIMBra, ".nc"))
    TamSerie <- dim(NamesDatanc)[3]
    NamesData <- matrix(0, TamSerie, 1)
    NamesData <-
      gsub("X", " ", gsub("[.]", "/", names(NamesDatanc)))
